...
def design_pnp_scene(scene_config, robot_config, show_viewer=True):
    scene = gs.Scene(
    ...
    if robot_config["asset"].endswith(".urdf"):
    ...
    active_pos   = (scene_config["object_active"]["default_position"]["x"] / 100,
                    ...
                    object_active_height / 2 + robot_config["robot_to_table_height"] / 100 / 2 + scene_config["object_active"]["height_thres"] / 100)
    ...
    passive_pos = (scene_config["object_passive"]["default_position"]["x"] / 100, 
                   ...
                   object_passive_height / 2 + robot_config["robot_to_table_height"] / 100 / 2)
...
def init_scene_physic_params(scene, scene_dict, scene_config, robot_config):
    ...
    physic_params = robot_config["physic_params"]
    robot.set_dofs_kp(kp=physic_params["kp"], dofs_idx_local=all_dof_ids[:])
    robot.set_dofs_kv(kv=physic_params["kv"], dofs_idx_local=all_dof_ids[:])
    robot.set_dofs_force_range([-100, -100], [100, 100], dofs_idx_local=all_dof_ids[-2:])
